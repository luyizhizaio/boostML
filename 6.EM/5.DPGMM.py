...
if __name__ == '__main__':
    ...
    plt.scatter(x[:, 0], x[:, 1], s=30, c=y, cmap=cm_dark, marker='o')

    #画中心区域
    clrs = list('rgbmy')
    for i, (center, cov) in enumerate(zip(centers, covs)):
        value, vector = sp.linalg.eigh(cov) #用于矩阵特征分解，返回特征值，特征向量
        width, height = value[0], value[1]
        v = vector[0] / sp.linalg.norm(vector[0]) #sp.linalg.norm 求向量二范数
        ...
        ax.add_artist(e) #A Circle 是 Artist的一个子类，通过add_artist增加
