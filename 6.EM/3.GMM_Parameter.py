...
if __name__ =="__main__":
    ...
    m = np.array(((1, 1), (1, 3)))
    x1 = x1.dot(m) #相当于把数据映射到其他空间。
    x2 = np.random.multivariate_normal(mean=(-1, 10), cov=cov1, size=N2)
    x = np.vstack((x1, x2))
    y = np.array([0] * N1 + [1] * N2)

    #covariance_type 四种取值
    ...
    ax = plt.axes() #相当于subplot
    b1 = ax.bar(xpos - 0.3, err, width=0.3, color='#77E0A0') #在子图上画一个bar
    b2 = ax.twinx().bar(xpos, bic, width=0.3, color='#FF8080') #在子图上画一个bar
    plt.grid(True)
